...
```{.javascript}
*특정 이벤트 처리기 동작 시키기
하나 이상의 요소에 대해 이벤트를 강제로 일으켜야 하는 경우가 있다.
이 요소는 하나 이상의 플러그인에 속해 있으며, 그렇기에 이벤트와 바인드된 이벤트 처리기를 가지고 있을 수 있다.

문제는 이런 이벤트가 click이나 mousedown과 같이 일반적인 이벤트인 경우이다. 
이런 경우 이벤트를 강제로 실행 시키는 것은 다른 이벤트 처리기들을 실행시킬 수 있다.
